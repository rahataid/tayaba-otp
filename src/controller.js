...
module.exports = {
   ...
   */
  async getContract() {
    const { data: { data: { abi } } } = await axios(`${rahatServer}/api/v1/misc/contracts/RahatClaim`);
    const {
      data: {
        data: {
  ...
  async addOtpToClaim(contract, claimId, otp) {
  ...
  async contractListen() {
    ...
    console.log('----------------------------------------');
    console.log(currentContract.listenerCount('ClaimCreated'), 'listeners');
  },
